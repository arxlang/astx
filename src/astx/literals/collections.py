...
class LiteralList(Literal):
    ...
    def __init__(self, elements: List[Literal], loc: SourceLocation = NO_SOURCE_LOCATION) -> None:
...
class LiteralTuple(Literal):
    ...
    def __init__(self, elements: Tuple[Literal, ...], loc: SourceLocation = NO_SOURCE_LOCATION) -> None:
...
class LiteralSet(Literal):
    ...
    def __init__(self, elements: Set[Literal], loc: SourceLocation = NO_SOURCE_LOCATION) -> None:
        """Initialize LiteralSet."""
        super().__init__(loc)
        self.elements = set(elements)
        unique_types = {type(elem.type_) for elem in elements}
        self.type_ = SetType(unique_types.pop()() if len(unique_types) == 1 else Int32())
        self.loc = loc
...
class LiteralDict(Literal):
    ...
    def __init__(self, elements: Dict[Literal, Literal], loc: SourceLocation = NO_SOURCE_LOCATION) -> None:
